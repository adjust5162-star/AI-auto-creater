...
export async function saveUploadedAsset(projectId, file) {
  ...
  const storagePath = await persistAssetBuffer(projectId, fileName, buffer, file.type || "application/octet-stream");
...
export async function saveBufferedAsset(projectId, asset) {
  ...
  const storagePath = await persistAssetBuffer(projectId, fileName, asset.buffer, mimeType);
...
export async function ensureAssetFile(projectId, asset) {
  const resolved = resolveAssetPath(projectId, asset);
  if (existsSync(resolved)) return resolved;
  if (!asset.storagePath || !hasSupabaseStore()) return resolved;

  await fs.mkdir(path.dirname(resolved), { recursive: true });
  const buffer = await downloadAssetObject(asset.storagePath);
...
async function persistAssetBuffer(projectId, fileName, buffer, mimeType) {
  if (!hasSupabaseStore()) return undefined;
  const storagePath = `projects/${projectId}/uploads/${fileName}`;
  await uploadAssetObject(storagePath, buffer, mimeType);
  return storagePath;
}
