...
function generateLocally(input) {
  if (hasBrokenPlaceholders(input.sourceText)) {
...
function generateFallbackScenes(input) {
  ...
  return normalizeScenes(
    [
      ...
      {
        headline: "장면 흐름 구성",
        narration: "원문 자료를 바탕으로 도입, 전개, 결론의 흐름을 만들고 자막과 내레이션을 자연스럽게 연결합니다.",
        highlightedWords: ["장면", "자막", "내레이션"]
      },
      {
        headline: "렌더링 품질 검증",
        narration: "최종 MP4 재생과 자막 표시를 확인해 배포 환경에서도 안정적으로 볼 수 있는 결과물을 완성합니다.",
        highlightedWords: ["MP4", "재생", "배포"]
      }
    ],
    input.targetDuration
  );
...
function hasBrokenPlaceholders(value) {
