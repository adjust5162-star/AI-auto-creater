...
export const remoteAssetLimit = 80 * 1024 * 1024;
export const remoteTextLimit = 5 * 1024 * 1024;
...
export async function importSourceUrl(projectId, sourceUrl) {
  ...
  if (!isAllowedRemoteUrl(url)) {
    return {
      assets: [],
      text: "",
      warnings: ["내부 네트워크 또는 로컬 주소는 보안을 위해 가져오지 않습니다."]
    };
  }
  ...
  try {
    ...
    if (isTextLike(type)) {
      const { buffer } = await readResponseBuffer(response, remoteTextLimit, { allowTruncate: true });
...
export function buildSourceText(rawText, sourceUrl, assets) {
  ...
  if (audioAssets.length) {
    parts.push("업로드된 오디오는 최종 영상의 배경음 또는 음성 트랙으로 사용합니다.");
  }
  if (sourceUrl) {
...
async function readResponseBuffer(response, limit, options = {}) {
  const allowTruncate = Boolean(options.allowTruncate);
  const length = Number(response.headers.get("content-length") || 0);
  if (length > limit && !allowTruncate) {
    throw new Error(`파일이 너무 큽니다. 최대 ${Math.round(limit / 1024 / 1024)}MB까지 지원합니다.`);
  }

  const reader = response.body?.getReader();
  if (!reader) {
    const buffer = Buffer.from(await response.arrayBuffer());
    if (buffer.byteLength <= limit) return { buffer, truncated: false };
    if (!allowTruncate) throw new Error(`파일이 너무 큽니다. 최대 ${Math.round(limit / 1024 / 1024)}MB까지 지원합니다.`);
...
function describeRemoteMedia(url, asset) {
  ...
  return `제공된 ${kind} URL ${url.href}를 장면의 시각 자료로 사용해 완성형 영상을 제작합니다.`;
}
