...
async function inlineOutput(project, jobId, outputFolder, finalPath, duration, spec) {
  return {
    videoUrl: await fileDataUrl(finalPath, "video/mp4"),
    srtUrl: await fileDataUrl(path.join(outputFolder, "captions.srt"), "text/plain;charset=utf-8"),
    vttUrl: await fileDataUrl(path.join(outputFolder, "captions.vtt"), "text/vtt;charset=utf-8"),
    projectJsonUrl: dataUrl(
      Buffer.from(JSON.stringify({ projectId: project.id, jobId, project }, null, 2), "utf8"),
...
async function renderSceneClip(ffmpegPath, project, scene, destination, sceneIndex, spec) {
  ...
  try {
    ...
    if (job) {
      job.warnings.push(`Scene ${scene.index} text overlay was skipped because FFmpeg drawtext failed.`);
      await saveJob(job);
    }
  }
}
