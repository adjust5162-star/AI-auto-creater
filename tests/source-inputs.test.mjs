...
test("extracts readable text from HTML source", () => {
  const html = `
    <html>
      <head><title>제품 소개 영상</title><meta name="description" content="신제품 핵심 기능"></head>
      <body><script>ignore()</script><h1>출시 안내</h1><p>빠르게 이해되는 영상 대본입니다.</p></body>
    </html>
  `;

  const text = extractReadableText(html, "text/html");
  assert.match(text, /제품 소개 영상/);
  assert.match(text, /신제품 핵심 기능/);
  assert.match(text, /빠르게 이해되는 영상 대본입니다/);
